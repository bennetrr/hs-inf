...
int child_process(const int sem_handle) {
    ...
    sem_wait(sem_handle, SEM_NOTIFY_CHILD);
    ...
    sem_signal(sem_handle, SEM_NOTIFY_PARENT);
...
int main(void) {
    const int sem_handle = sem_create(2);
    if (sem_handle == -1) {
    ...
    sem_signal(sem_handle, SEM_NOTIFY_CHILD);

    printf("Parent: Waiting for notification from child\n");
    sem_wait(sem_handle, SEM_NOTIFY_PARENT);
